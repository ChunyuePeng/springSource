...
public abstract class AbstractPlatformTransactionManager implements PlatformTransactionManager, Serializable {
	...
	public final TransactionStatus getTransaction(@Nullable TransactionDefinition definition)
		...
		//Spring默认使用spring jdbc的事务，所以这里会创建DataSourceTransactionObject，并从事务管理器中取出
		//ConnectionHolder赋值给DataSourceTransactionObject对象的属性
		Object transaction = doGetTransaction();
		boolean debugEnabled = logger.isDebugEnabled();

		//判断是否已存在事务：根据DataSourceTransactionObject.connectionHolder是否为空判断
		...
		//如果是PROPAGATION_REQUIRED、PROPAGATION_REQUIRES_NEW、PROPAGATION_NESTED这三种类型将开启新事务
	...
	protected final SuspendedResourcesHolder suspend(@Nullable Object transaction) throws TransactionException {
		//如果事务是激活的，且当前事务同步机制也是激活的
		if (TransactionSynchronizationManager.isSynchronizationActive()) {
			//挂起当前线程中所有同步的事务
			List<TransactionSynchronization> suspendedSynchronizations = doSuspendSynchronization();
			try {
				Object suspendedResources = null;
				//将挂起的事务操作交由具体的事务处理器处理
				if (transaction != null) {
		...
		//如果事务是激活的，但是事务同步机制不是激活的，则只需要保存事务状态，
		//不需要重置事务相关的局部变量
			...
			try {
				...
				//新事务的回滚
					...
					if (status.hasTransaction()) {
						//适用于REQUIRED传播级别，这里并不会真的回滚，而是设置回滚
						//标志位
